...
b.  Inspect the ACF and PACF plots for the time series. What ARMA model would be appropriate for the time series?
c.  Fit an ARMA model, allowing for automatic model selection. What is the order of the model selected?
d.  Plot the forecast for the model.
e.  What are the model parameters?
f.  What are the roots of the AR polynomial? Does this agree with the periodicity you observe?
g.  Fit a Holt-Winters method with seasonal period = 10.
h.  Compare the accuracy of the two methods.
...
d.  Using this orthogonality condition, understand the geometric interpretation of partial correlation [here](https://en.wikipedia.org/wiki/Partial_correlation#Geometrical).
e.  Prove the regression coefficient interpretation of the PACF, i.e. equation (12.4) in the notes.

## 4. AIC

a.  What goes wrong when we choose $p$ and $q$ to be too big?
b.  What is the expectation of the log likelihood?
c.  Given an ARMA($p_0$,$q_0$) model, what is the expectation of AIC for the choice $p \geq p_0$ and $q \geq q_0$?
d.  Given an ARMA($p_0$,$q_0$) model, what is the expectation of AIC for the choice $p < p_0$ or $q < q_0$?
